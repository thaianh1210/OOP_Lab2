...
public class CartScreenController {
    ...
    public void setCartAndStore(Cart cart, Store store) {
        this.cart = cart;
        this.store = store;
        initialize();
        updateTotalCost();
    ...
    private void handleViewStore(ActionEvent event) {
        try {
            // Tạo StoreScreen với store và cart
            StoreScreen storeScreen = new StoreScreen(store, cart);
            Stage stage = new Stage();
            storeScreen.start(stage);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void exitScreen(ActionEvent event) {
        // Lấy Stage hiện tại từ một nút hoặc bất kỳ phần tử nào trên giao diện
